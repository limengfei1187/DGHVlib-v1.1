 ...
 */
 ...
 void c_get_sc_ciphdivp_lsb(__cit* cc, __cit* ciph, __sc_pubkey_set* pubkey, __sec_setting* para){
     ...
     for(i = 0; i < pubkey->s0_group_cnt; i++){
         for(j = 0; j < pubkey->s1_group_cnt; j++){
             if(j == pubkey->s1_group_cnt - 1){
                 length = pubkey->last_group_length;
             }else{
                 length = pubkey->every_group_length;
             }
             for(k = 0; k < pubkey->every_group_length; k++){
                 for(l = 0; l < length; l++){
                     set_ev_table(index, ciph->z[index], ev_table);
                     if(t < para->theta){
                         mpz_mul(si, pubkey->s0_vector[i *  pubkey->every_group_length + k], pubkey->s1_vector[j *  pubkey->every_group_length + l]);
                         t++;
                     }else{
                         mpz_set(si,pubkey->s_fills[0]);
                     }

                     for(x = 0; x < ev_table->y; x++){
                         mpz_mul(ev_table->table[index][x], ev_table->table[index][x], si);
                     }
                     index++;
                 }
             }
     ...
     for(i = ev_table->y - 1; i > 0; i--){
         c_get_hw(i, ev_table, para, pubkey->x0);
     }
 ...
 void sc_bootstrap(__cit* cc, __cit* ciph, __sc_pubkey_set* pubkey, __sec_setting* para, randstate rs){
     ...
     init_cit(&c1, para->Theta);
     init_cit(&c2, para->Theta);
     c_get_sc_ciph_lsb(c1, ciph, pubkey, para, rs);
     c_get_sc_ciphdivp_lsb(c2, ciph, pubkey, para);
     evaluate_add(cc, c1, c2, pubkey->x0);
     clear_cit(c1);
     clear_cit(c2);
 }
